...
export default function getCurrentSheetStatus(record: recordType) {
  ...
  const totalTookMedicineLength = record.dailyRecord.filter(
    (item) => item.tookMedicine === true
  ).length;

  let tookDays =
    (totalTookMedicineLength + beginSheetIndex) % numOfPillsPerSheet;

  const isTodayTookMedicine = record.dailyRecord[0].tookMedicine;
  tookDays =
    tookDays === 0 && isTodayTookMedicine ? numOfPillsPerSheet : tookDays;
...
export function hasNoRecordDays(record: recordType, offset = 0) {
  ...
  const truncatedDailyRecordWithoutToday = trimedDairyRecord.slice(
    1,
    startTakeMedicineIndex
  );
  ...
  const hasNoRecordWithoutToday = truncatedDailyRecordWithoutToday.some(
    (record) => {
...
export function countTotalTakeMedicineDays(record: recordType) {
  const takeMedicineDays = record.dailyRecord.filter(
    (dailyRecord) => dailyRecord.tookMedicine
  ).length;
  const isTodayTookMedicine = record.dailyRecord[0].tookMedicine;
